...
describe('cartService', () => {
    ...
    describe('updateCartItem', () => {
        it('should call apiClient.put with correct endpoint', async () => {
            const update = { productCode: 'P1', quantity: 2 };
            // NOTE: Service signature takes UpdateCartItemRequest which includes productCode
            const mockResponse = { success: true, data: true };
            mockPut.mockResolvedValue(mockResponse);

            const result = await cartService.updateCartItem(update);

            expect(mockPut).toHaveBeenCalledWith('/carts/items', update);
            expect(result).toBe(mockResponse);
        });
    });
