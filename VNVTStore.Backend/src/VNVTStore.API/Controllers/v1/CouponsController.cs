...
public class CouponsController : BaseApiController<CouponDto, CreateCouponDto, CouponDto>
{
    ...
    [HttpGet]
    public async Task<IActionResult> GetAllCoupons([FromQuery] int pageIndex = AppConstants.Paging.DefaultPageNumber, [FromQuery] int pageSize = AppConstants.Paging.DefaultPageSize)
    {
        var result = await Mediator.Send(new GetAllCouponsQuery(pageIndex, pageSize));
        return HandleResult(result);
    }

    protected override IRequest<Result<PagedResult<CouponDto>>> CreatePagedQuery(int pageIndex, int pageSize, string? search, SortDTO? sort, List<SearchDTO>? filters)
        => new GetPagedQuery<CouponDto>(pageIndex, pageSize, search, sort, filters);
