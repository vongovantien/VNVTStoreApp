...
const OrderSuccessPage = () => {
  ...
  // Redirect immediately if there is no order code in the URL
  useEffect(() => {
    if (!orderCode) {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (!res.success || !res.data) {
        // Treat a failed fetch as an error so the error branch below handles redirect
        throw new Error(res.message || 'Order not found');
      }
      return res.data;
    },
    enabled: !!orderCode,
    retry: false, // Don't retry — a 404 should redirect, not spin
    staleTime: 5 * 60 * 1000, // Cache for 5 minutes; the order won't change on this page
  });

  // Redirect on fetch error (order not found / no access)
  useEffect(() => {
    if (isError) {
      navigate('/products', { replace: true });
    }
  ...
  // Use customerEmail if available; fall back to a generic label.
  // Never show userCode (internal ID) to the customer.
  const displayEmail = order.customerEmail || t('order.yourEmail', 'email của bạn');
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7">
            <div className="flex flex-col sm:flex-row items-start gap-5 mb-8 text-left">
              ...
              <div className="mt-2">
                <h1 className="text-2xl font-bold text-[#333333] mb-2">
                  {t('order.thankYou', 'Cảm ơn bạn đã đặt hàng')}
                </h1>
                <p className="text-[#666666] text-sm leading-relaxed">
                  {t('order.emailConfirmation', 'Một email xác nhận đã được gửi tới')}{' '}
                  <span className="text-[#333333]">{displayEmail}</span>.
                  <br />
                  {t('order.checkEmail', 'Xin vui lòng kiểm tra email của bạn')}
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#F0F2F5] p-6 sm:p-8 rounded-sm grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-8 text-left border border-gray-100">
              {/* Buyer Info */}
              <div>
                <h3 className="text-[15px] font-medium text-[#333333] mb-2">
                  {t('order.buyerInfo', 'Thông tin mua hàng')}
                </h3>
                <div className="text-[#666666] text-[13px] space-y-1">
                  <p>{order.shippingName || order.customerName || t('order.guest', 'Khách lẻ')}</p>
              ...
              <div>
                <h3 className="text-[15px] font-medium text-[#333333] mb-2">
                  {t('order.shippingAddress', 'Địa chỉ nhận hàng')}
                </h3>
                <div className="text-[#666666] text-[13px] space-y-1">
                ...
                <div className="text-[#666666] text-[13px]">
                  <p>
                    {order.paymentMethod === 'COD'
                      ? t('paymentMethods.cod', 'Thanh toán khi giao hàng (COD)')
                      : t(`paymentMethods.${order.paymentMethod?.toLowerCase()}`, order.paymentMethod ?? '')}
                  </p>
                </div>
              </div>
              ...
              <div>
                <h3 className="text-[15px] font-medium text-[#333333] mb-2">
                  {t('order.shippingMethod', 'Phương thức vận chuyển')}
                </h3>
                <div className="text-[#666666] text-[13px]">
                  <p>
                    {order.shippingFee === 0
                      ? t('order.freeShip', 'FREE SHIP')
                      : t('order.standardShipping', 'Giao hàng tiêu chuẩn')}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-5">
            <div className="bg-white rounded-sm shadow-sm border border-gray-200 overflow-hidden">
              <div className="px-5 py-3 border-b border-gray-100 bg-white">
                <h3 className="font-bold text-[#333333] text-[15px]">
                  {t('order.order', 'Đơn hàng')} #{order.code} ({order.orderItems?.length ?? 0})
                </h3>
              </div>
              <div className="p-5">
